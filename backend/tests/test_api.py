...
def test_list_and_filter(sandbox):
    ...
    resp = sandbox.client.get("/items", params={"category": "recipe"})
    assert [i["queue_id"] for i in resp.json()] == ["b"]

    resp = sandbox.client.get("/items", params={"status": "enriched"})
    assert [i["queue_id"] for i in resp.json()] == ["a"]


def test_get_item(sandbox):
    sandbox.seed(queue_id="a")
    resp = sandbox.client.get("/items/a")
    assert resp.status_code == 200
...
def test_get_item_404(sandbox):
    resp = sandbox.client.get("/items/does-not-exist")
...
def test_patch_updates_fields_and_persists_to_disk(sandbox):
    ...
    resp = sandbox.client.patch("/items/a", json={"title": "New title"})
    assert resp.status_code == 200
...
def test_patch_404(sandbox):
    resp = sandbox.client.patch("/items/nope", json={"title": "x"})
    assert resp.status_code == 404


def test_move_archive(sandbox):
    sandbox.seed(queue_id="a")
    resp = sandbox.client.post("/items/a/move", json={"action": "archive"})
    assert resp.status_code == 200
...
def test_move_dismiss(sandbox):
    sandbox.seed(queue_id="a")
    resp = sandbox.client.post("/items/a/move", json={"action": "dismiss"})
    assert resp.status_code == 200
...
def test_move_keep_writes_vault_note_and_archives(sandbox):
    ...
    resp = sandbox.client.post("/items/a/move", json={"action": "keep"})
    assert resp.status_code == 200
...
def test_move_keep_unmapped_category_falls_back_to_unclassified(sandbox):
    ...
    sandbox.client.post("/items/a/move", json={"action": "keep"})
    assert list((sandbox.vault_dir / "Unclassified").glob("*.md"))


def test_move_already_archived_is_409(sandbox):
    sandbox.seed(queue_id="a")
    sandbox.client.post("/items/a/move", json={"action": "archive"})
    resp = sandbox.client.post("/items/a/move", json={"action": "archive"})
    assert resp.status_code == 409


def test_move_missing_item_is_404(sandbox):
    resp = sandbox.client.post("/items/nope/move", json={"action": "archive"})
    assert resp.status_code == 404


def test_move_unknown_action_is_400(sandbox):
    sandbox.seed(queue_id="a")
    resp = sandbox.client.post("/items/a/move", json={"action": "delete"})
